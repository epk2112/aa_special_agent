...
if [ ! -f "Special_Mockups_Gen_agent.py" ]; then
    echo "Error: Special_Mockups_Gen_agent.py not found in the current directory." | tee -a $LOG_FILE
    exit 1
fi
...
# Pull the Docker image once before starting containers
echo "Pulling Docker image epk2112/jupyter-environment:latest..." | tee -a $LOG_FILE
docker pull epk2112/jupyter-environment:latest 2>&1 | tee -a $LOG_FILE

# Check if pull was successful
PULL_EXIT_CODE=${PIPESTATUS[0]}
if [ $PULL_EXIT_CODE -ne 0 ]; then
    echo "Failed to pull Docker image with exit code $PULL_EXIT_CODE" | tee -a $LOG_FILE
    exit 1
fi
...
for i in $(seq 1 $NUM_CONTAINERS); do
    cat >> docker-compose.yml << EOF
  jupyter-$i:
    image: epk2112/jupyter-environment:latest
    container_name: jupyter-environment-instance-$i
    cpus: 4
    volumes:
      - ./Special_Mockups_Gen_agent.py:/home/jupyter_user/documents/Special_Mockups_Gen_agent.py
    ...
    command: >
      bash -c "
        ...
        git clone https://github.com/epk2112/doc_content_gen.git &&
        rm -rf /home/jupyter_user/documents/doc_content_gen/special_mockups_agent/bulk_pngLogos_dir/* &&
        cp -r /home/jupyter_user/source_pngs/* /home/jupyter_user/documents/doc_content_gen/special_mockups_agent/bulk_pngLogos_dir/ &&
        python /home/jupyter_user/documents/Special_Mockups_Gen_agent.py > /home/jupyter_user/logs/python_output_$i.log 2>&1
      "
    tty: true
